...
import createConfig from '../../lib/presets/nestjs.mjs';
...
describe('nestjs config', () => {
  ...
  describe('with jest (default)', () => {
    ...
    test('uses commonjs sourceType', () => {
      expect(preset.languageOptions.sourceType).toBe('commonjs');
    });
    ...
    test('disables @typescript-eslint/no-explicit-any for NestJS patterns', () => {
      expect(typescriptEntry.rules['@typescript-eslint/no-explicit-any']).toBe('off');
    });

    test('warns on floating promises', () => {
      expect(typescriptEntry.rules['@typescript-eslint/no-floating-promises']).toBe('warn');
    });

    test('warns on unsafe argument usage', () => {
      expect(typescriptEntry.rules['@typescript-eslint/no-unsafe-argument']).toBe('warn');
    });
  ...
  describe('with both tsConfig and testRunner', () => {
    const config = createConfig({ tsConfig: 'tsconfig.vitest.json', testRunner: 'vitest' });
    ...
    test('uses custom tsConfig in typescript config', () => {
      ...
      expect(tsEntry.languageOptions.parserOptions).toEqual({ project: ['tsconfig.vitest.json'] });
    });
  });
});
